import pickle
import os
data = [['Гречневая каша','4.5','1.6','27.4','137'],['Яйцо куриное','12.9','11.1','0.6','153'],['Колбаса вареная Докторская','13.4',
             '22.9','0','257'],['Кефир 2,5%','3','2.5','4','51'],['Говядина','18.7','12.6','0','191'],
            ['Индейка','21.1','12.3','0','192'],['Яйцо перепелиное','11.9','13.3','0.9','170'],
        ['Шпинат','2.5','0','2.6','22'],['Фундук','16.3','66.7','9.8','701'],
    ...
    if a.isdigit()== False or int(a) > 7 or int(a) < 0:
     ...
     '\u2500'*30+'\u2534'+('\u2500'*10+'\u2534')*3+'\u2500'*10+'\u2518')
        ...
        if a == '6':
            ...
            for i in range(len(data)):
                """
                print(((32-len(data_new[i][0]))//2+(32-len(data_new[i][0]))%2)*' '+data_new[i][0]+((32-len(data_new[i][0]))//2)*' '+
                ((11-len(data_new[i][1]))//2+(11-len(data_new[i][1]))%2)*' '+data_new[i][1]+((11-len(data_new[i][1]))//2)*' '+
                ((10-len(data_new[i][2]))//2+(10-len(data_new[i][2]))%2)*' '+data_new[i][2]+((10-len(data_new[i][2]))//2)*' '+
                ((11-len(data_new[i][3]))//2+(11-len(data_new[i][3]))%2)*' '+data_new[i][3]+((11-len(data_new[i][3]))//2)*' '+
                ((11-len(data_new[i][4]))//2+(11-len(data_new[i][4]))%2)*' '+data_new[i][4]+((11-len(data_new[i][4]))//2)*' ')
                 """
                print(((32-len(data[i][0]))//2+(32-len(data[i][0]))%2)*' '+data[i][0]+((32-len(data[i][0]))//2)*' '+
                ((11-len(data[i][1]))//2+(11-len(data[i][1]))%2)*' '+data[i][1]+((11-len(data[i][1]))//2)*' '+
                ((10-len(data[i][2]))//2+(10-len(data[i][2]))%2)*' '+data[i][2]+((10-len(data[i][2]))//2)*' '+
